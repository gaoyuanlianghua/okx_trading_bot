...
class StrategyMonitor:
    ...
    def update_strategy_status(self, strategy_name: str, status: str):
        ...
        if strategy_name in self.strategies:
            ...
            if (
                status == "running"
                and self.strategies[strategy_name]["start_time"] is None
            ):
        ...
        if strategy_name in self.strategies:
            ...
            if strategy_metrics.total_trades > 0:
                strategy_metrics.win_rate = (
                    strategy_metrics.win_trades / strategy_metrics.total_trades
                )
                strategy_metrics.avg_profit_per_trade = (
                    strategy_metrics.total_profit / strategy_metrics.total_trades
                )

            # 更新运行时间
            if strategy_data["start_time"]:
                strategy_metrics.strategy_uptime = (
                    time.time() - strategy_data["start_time"]
                )
            ...
            if len(strategy_data["performance_history"]) > 1000:
                strategy_data["performance_history"] = strategy_data[
                    "performance_history"
                ][-1000:]
    ...
    def record_execution_time(self, strategy_name: str, execution_time: float):
        ...
        if strategy_name in self.strategies:
            ...
            if strategy_data["metrics"].execution_count > 0:
                strategy_data["metrics"].average_execution_time = sum(
                    strategy_data["execution_times"]
                ) / len(strategy_data["execution_times"])

            # 限制执行时间记录长度
            if len(strategy_data["execution_times"]) > 100:
                strategy_data["execution_times"] = strategy_data["execution_times"][
                    -100:
                ]
    ...
    def get_strategy_trade_history(
    ...
    def _perform_monitoring(self):
        ...
        for strategy_name, data in self.strategies.items():
            ...
            if time_since_update > 60 and data["status"] == "running":
                logger.warning(
                    f"策略长时间未更新: {strategy_name} ({time_since_update:.1f}秒)"
                )

            # 检查执行时间是否过长
            if data["metrics"].average_execution_time > 1.0:
                logger.warning(
                    f"策略执行时间过长: {strategy_name} ({data['metrics'].average_execution_time:.2f}秒)"
                )

        # 记录整体状态
        overall = self.get_overall_metrics()
        logger.debug(
            f"监控状态: 策略数={overall['total_strategies']}, 运行中={overall['running_strategies']}, 总交易数={overall['total_trades']}, 总盈亏={overall['total_profit']:.2f}"
        )
