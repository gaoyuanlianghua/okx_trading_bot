...
class PassivbotIntegrator(BaseStrategy):
    ...
    def __init__(self, api_client=None, config=None):
        ...
        if not self.api_keys_path:
            self.api_keys_path = os.path.join(
                os.path.dirname(__file__), "passivbot", "api-keys.json"
            )
    ...
    def download_market_data(
        self, symbol=None, timeframe="1h", start_time="2024-01-01", end_time=None
    ):
        """下载市场数据用于回测"""
        symbol = symbol or self.default_config["symbol"]

        logger.info(
            f"开始下载市场数据: {symbol}, {timeframe}, {start_time} → {end_time}"
        )

        try:
            # 使用命令行调用passivbot的下载功能
            passivbot_script = os.path.join(
                os.path.dirname(__file__), "passivbot", "src", "main.py"
            )
            cmd = [
                sys.executable,
                passivbot_script,
    ...
    def create_default_config(self, symbol="BTC-USDT-SWAP", output_path=None):
        ...
        output_path = output_path or os.path.join(
            os.path.dirname(__file__), "passivbot_config.hjson"
        )
    ...
    def stop_trading(self, bot_instance):
        ...
        if (
            bot_instance
            and isinstance(bot_instance, dict)
            and "process" in bot_instance
        ):
            try:
                # 尝试停止机器人进程
