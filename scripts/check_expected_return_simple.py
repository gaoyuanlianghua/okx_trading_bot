#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# check_expected_return_simple.py
# 查看订单智能体收益预测因子变化（简单版）
...
class ExpectedReturnAnalyzer:
    ...
    def analyze_threshold(self):
        ...
        above_threshold = len([r for r in self.expected_returns if r >= threshold])
        below_threshold = len([r for r in self.expected_returns if r < threshold])
        
        logger.info(f"阈值分析 (阈值: {threshold*100:.2f}%):")
        logger.info(f"高于阈值: {above_threshold}次")
        logger.info(f"低于阈值: {below_threshold}次")
        
        if above_threshold > 0:
            max_return = max(self.expected_returns)
            max_price = self.timestamps[self.expected_returns.index(max_return)]
            logger.info(f"最大预期收益率: {max_return:.4f} ({max_return*100:.2f}%) at {max_price:.2f} USDT")
