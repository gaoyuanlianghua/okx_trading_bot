...
class TradingBot:
    ...
    def __init__(self, config_path="config/okx_config.json"):
        ...
        # Run network auto-adaptation first
    ...
    def start(self, use_gui=True):
        ...
        try:
            ...
            # 在后台线程中初始化和启动智能体，避免阻塞GUI
            from PyQt5.QtCore import QThread, QObject, pyqtSignal
            
            class AgentInitStartWorker(QObject):
                """智能体初始化和启动工作线程"""
                finished = pyqtSignal()
                ...
                def run(self):
                    ...
                    try:
                        # 初始化智能体
                        self.trading_bot.init_agents()
                        
                        # 启动智能体
                        self.trading_bot.start_agents()
                        
                        # 订阅默认交易对
                        decision_agent = self.trading_bot.agents.get("decision_coordination_agent")
                        if decision_agent:
                            decision_agent.add_symbol_subscription(self.trading_bot.config.get("symbol", "BTC-USDT-SWAP"))
                        
                        # 激活默认策略
                        strategy_agent = self.trading_bot.agents.get("strategy_execution_agent")
                        if strategy_agent:
                            strategy_agent.activate_strategy(self.trading_bot.config.get("strategy", "DynamicsStrategy"))
                    except Exception as e:
            ...
            worker = AgentInitStartWorker(self)
            worker.moveToThread(agent_thread)
            agent_thread.started.connect(worker.run)
            worker.finished.connect(agent_thread.quit)
            worker.finished.connect(worker.deleteLater)
            agent_thread.finished.connect(agent_thread.deleteLater)
            agent_thread.start()
